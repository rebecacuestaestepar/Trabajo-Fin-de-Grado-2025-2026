...
\begin{enumerate}
	...
	\item \textbf{Consulta de reservas:} La aplicación incluye un apartado de consulta de reservas, que permite visualizar y filtrar todas las reservas puntuales registradas en el sistema por responsable, motivo y aula, para facilitar su seguimiento y control.
...
\begin{enumerate}
	\item \textbf{Diseñar el modelo de bases de datos relacional:} se ha diseñado para permitir el almacenamiento de forma estructurada de la información relativa a aulas, grados, asignaturas, reservas, horarios, exámenes y usuarios. Esta estructura permite que se pueda extrapolar a diferentes facultades en un futuro.
	\item \textbf{Implementar la arquitectura web:} su implementación se ha basado en la separación entre el \textit{frontend} y el \textit{backend}, favoreciendo de esta forma la modularidad y mantenibilidad
	\item \textbf{Implementar sistema de permisos:} se ha implementado un sistema de control de acceso basado en roles que limita las acciones disponibles en la aplicación según el tipo de usuario.
	\item \textbf{Calcular la disponibilidad:} que garantiza la actualización de manera automática de la ocupación de las aulas ante cualquier cambio que se realice en las reservas o los horarios.
	\item \textbf{Incorporar mecanismos de parseo y análisis de documentos existentes:} se ha planteado la incorporación de procesos de análisis sobre documentos ya existentes, con el fin de facilitar la carga masiva y la migración de datos al sistema.
	\item \textbf{Validar restricciones:} al realizar una acción se comprueba que no viole ninguna de las restricciones definidas.
	\item \textbf{Calidad:} se realizan validaciones y manejo de errores con el fin de verificar el correcto funcionamiento de la aplicación.
\end{enumerate}
